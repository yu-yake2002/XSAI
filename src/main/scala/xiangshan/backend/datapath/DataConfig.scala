...
object DataConfig {
  ...
  case class MxData() extends DataConfig("mx", 64)
  case class FakeIntData() extends DataConfig("fakeint", 64)
  case class NoData() extends DataConfig("nodata", 0)

  def RegSrcDataSet   : Set[DataConfig] = Set(IntData(), FpData(), VecData(), V0Data(), VlData(), MxData())
  ...
  def MxRegSrcDataSet : Set[DataConfig] = Set(MxData())
