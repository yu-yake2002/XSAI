...
class IssueQueueMemAddrImp(override val wrapper: IssueQueue)(implicit p: Parameters, params: IssueBlockParams)
  ...
  io.wakeupToIQ.zip(params.exuBlockParams).zipWithIndex.foreach { case ((wakeup, param), i) =>
    if (param.hasLoadExu) {
      ...
      wakeup.bits.mtilexWen := (if (params.writeMtilexRf) RegNext(uop.bits.mtilexWen && uop.fire) else false.B)
      ...
      wakeup.bits.mtilexWenCopy.foreach(_.foreach(_ := (if (params.writeMtilexRf) RegNext(uop.bits.mtilexWen && uop.fire) else false.B)))
